...
+ 项目设置面板
  ...
  * 模块化选择，若项目为模块化项目请进行勾选。勾选后会自动对目录进行检测，如果检测不通过会发送windows通知`本项目不符合模块化项目结构`,如图 06所示。如通过，则会展示当前项目的所有模块并默认勾选，用户可根据需求自行勾选需要处理的模块，如图07所示。

  * 项目类型选择，此处目前提供`普通`和`rhyton`两种，默认为普通项目 。
